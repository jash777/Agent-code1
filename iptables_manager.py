...
class IPTablesManager:
   @staticmethod
    ...
    @staticmethod
    async def block_port(port: int, protocol: str = "tcp", chain: str = "INPUT") -> bool:
        try:
            success = await IPTablesManager.add_rule(
                protocol=protocol,
                port=port,
                action="DROP",
                chain=chain
            ...
            if success:
                logger.info(f"Successfully blocked {protocol} port {port} on chain {chain}")
            else:
                logger.error(f"Failed to block {protocol} port {port} on chain {chain}")
            return success
        except Exception as e:
            logger.error(f"Error in block_port: {e}")
            return False

    @staticmethod
    async def allow_port(port: int, protocol: str = "tcp", chain: str = "INPUT") -> bool:
        try:
            success = await IPTablesManager.add_rule(
                protocol=protocol,
                port=port,
            ...
            if success:
                logger.info(f"Successfully allowed {protocol} port {port} on chain {chain}")
            else:
                logger.error(f"Failed to allow {protocol} port {port} on chain {chain}")
            return success
        except Exception as e:
            logger.error(f"Error in allow_port: {e}")
            return False
